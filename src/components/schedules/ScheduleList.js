...
class ScheduleList extends React.Component {
  ...
  editDeleteDay({ event }) {
    return (
      <span>
        ...
        <br />
        <Link
          to={'/schedules/edit/' + event.id}
          className="ui small blue button"
        >
          Edit
        </Link>
        <Link
          to={'/schedules/delete/' + event.id}
  ...
  onMonthChange = (date, view) => {
    ...
    this.props.schedules.forEach(schedule => {
      let month = moment(moment.utc(schedule.Date).toDate()).format(
        'MMM, YYYY'
      );
    ...
    if (this.props.isSignedIn && this.props.currentUserId) {
      return (
        <div>
          ...
          <div className="ui celled list">{this.renderCalendar()}</div>
        </div>
      );
    } else {
